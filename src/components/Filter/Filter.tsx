...
import { selectFilter, setFilter, toggleDirection } from "../../redux/slice/filterSlice";
import { useAppDispatch } from "../../redux/store";
import style from "./Filter.module.scss";

const filter = ['Order', 'Price', 'Views']
...
const Filter: React.FC = () => {
  ...
  const {filter: activeFilter, direction} = useSelector(selectFilter)
  ...
  return (
    <div ref={filterRef} className={style.wrapper}>
      ...
      <div onClick={() => setOpen((prev) => !prev)} className={style.filter}>
        ...
        <span>Filter:</span>
        <span className={style.name_filter}>{filter[activeFilter]}</span>
      </div>
      {open && (
        <ul>
          {filter.map((item, id) => {
            return <li key={id} className={id === activeFilter ? style.active : ''} onClick={() => changeFilter(id)}>{item}</li>
          })}
        </ul>
      )}
    </div>
